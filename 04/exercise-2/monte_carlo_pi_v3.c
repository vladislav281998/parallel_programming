...
int main(int argc, char** argv) {
    ...
    count_t points_in_circle[num_threads * CACHE_LINE_SIZE / sizeof(count_t)];

    for (uint32_t i = 0; i < num_threads; ++i) {
        points_in_circle[i * CACHE_LINE_SIZE / sizeof(count_t)] = 0;
        inputs[i] = (struct MonteCarloPiInput) {
                .id = i,
                .iterations = per_thread_iterations + ((i == 0) ? (total_iterations % num_threads) : 0),
                .points_in_circle = &points_in_circle[i * CACHE_LINE_SIZE / sizeof(count_t)]
        };
    ...
    count_t total_points_in_circle = 0;
    for (uint32_t i = 0; i < num_threads; ++i) {
        if (creation_statuses[i] == 0) {
            ...
            if (status != 0) {
                ...
                total_points_in_circle += points_in_circle[i * CACHE_LINE_SIZE / sizeof(count_t)];
            }
        }
    }

    double pi_approximation = 4.0 * (total_points_in_circle / (double) total_iterations);
